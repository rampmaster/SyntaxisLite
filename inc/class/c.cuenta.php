...
class tsCuenta {
	 ...
	 * Actividad de sesión
	*/
	function ActividadSesion() {
		global $tsCore, $tsUser;

		$usuario = (int)$tsUser->uid;

		$session = db_exec(array(__FILE__, __LINE__), 'query', "SELECT user_lastlogin, user_registro FROM u_miembros WHERE user_id = " . $usuario);
		$session = db_exec('fetch_assoc', $session);

		$_SESSION['sessions'] = array(
			'tipo' 		=> ($detect->isMobile) ? 'smartphone' : 'monitor',
			'usuario' 	=> gethostname(),
			'navegador' => $tsCore->getUserAgent(),
			'clave' 		=> uniqid(mt_rand(), true),
			'ip' 			=> $tsCore->getIP(),
			'actividad' => $session["user_lastlogin"],
			'registro' => $session["user_registro"],
			'system' 	=> PHP_OS_FAMILY .' '. php_uname('r')
		);
		return $_SESSION['sessions'];
 	}	
    /*
        loadExtras()
    */
