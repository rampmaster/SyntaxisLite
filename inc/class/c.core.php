...
class tsCore {
	...
	*/
	function getCategorias() {
		// CONSULTA
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT cid, c_orden, c_nombre, c_seo FROM p_categorias ORDER BY c_orden');
		// GUARDAMOS
   ...
   */
   function getDomain(){
   	$domain = str_replace(array('https://','http://'), '', $this->settings['url']);
	...
	*/
	function currentUrl(){
    	$current_url_protocol 	 = $this->HTTPS_HTTP();
		$current_url_domain 		 = $_SERVER['HTTP_HOST'];
		$current_url_path 		 = $_SERVER['REQUEST_URI'];
		$current_url_all_include = $current_url_protocol . $current_url_domain . $current_url_path;
		$current_url_encode		 = urlencode($current_url_all_include); 
		return $current_url_encode;
	}
	/*
		setJSON($tsContent)
	*/
   ...
   */
   function extraer_img($texto) {
      ...
      if(empty($img)) $img = 'https://i2.wp.com/www.bicifan.uy/wp-content/uploads/2016/07/Producto-sin-imagen.jpg?fit=400%2C400&ssl=1';
      //
      return $img;
   }	
   function nobbcode($nobbcode = ''){
		$nobbcode = preg_replace('/\[([^\]]*)\]/', '', $nobbcode); 
		$regex = "@(https?://([-\w\.]+[-\w])+(:\d+)?(/([\w/_\.#-]*(\?\S+)?[^\.\s])?)?)@";
		$nobbcode = preg_replace($regex, ' ', $nobbcode);
		return $nobbcode;
	}
	function truncate($string = '', $can = NULL){
		$stc = ($can == '') ? '150' : $can;
	...
	function timeseo($time = '') {
		$time = date('Y-m-d\TH:i:s\Z', $time);
		return $time;
	}
	function convertirms($segundos) {
