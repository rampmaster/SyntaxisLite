...
class tsUpload
{
   ...
   public $allow_types = array('png', 'gif', 'jpeg'); // ARCHIVOS PERMITIDOS
   public $found       = 0; // VARIABLE BANDERA
   public $file_url    = ''; // URL
   public $file_size   = array(); // TAMA�O DEL ARCHIVO REMOTO
   public $image_size  = array('w' => 570, 'h' => 450);
   ...
   :: $type => URL o ARCHIVO
    */
   public function newUpload($type = 1)
   {
      $this->type = $type;
      // ARCHIVOS
      if ($this->type == 1) {
   ...
   public function uploadFile($file, $type = 'file') {
      ...
      $titulo = strtolower(str_replace(' ', '', $tsCore->settings['titulo']));
      // VALIDAR
   ...
   public function uploadUrl()
   {
      ...
      if (!empty($error)) {
         ...
         return array(1, urldecode($this->file_url));
      }
   ...
   public function cropAvatar($key) {
      ...
      $titulo = strtolower(str_replace(' ', '', $tsCore->settings['titulo']));
      $source = TS_FILES . 'uploads/'.$titulo.'_' . $_POST['key'] . '.' . $_POST['ext'];
      ...
      imagewebp($avatar, $root . '.webp', 100);
      imagedestroy($img);
      imagedestroy($avatar);
   ...
   public function uploadImagen($params) {
      ...
      if ($this->server == 'imgshack') curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
      // RESULTADO
      $result = curl_exec($ch);
      curl_close($ch);
      return $result;
   ...
   */
   function sexuser() {
      ...
      $query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT user_sexo FROM u_perfil WHERE user_id = {$tsUser->uid} LIMIT 1");
