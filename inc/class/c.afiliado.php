...
class tsAfiliado {
    ...
    */
    function newAfiliado(){
        ...
        //
		if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `w_afiliados` (a_titulo, a_url, a_banner, a_descripcion, a_date) VALUES (\''.$tsCore->setSecure($dataIn['titulo']).'\', \''.$tsCore->setSecure($dataIn['url']).'\', \''.$tsCore->setSecure($dataIn['banner']).'\', \''.$tsCore->setSecure($dataIn['desc']).'\', \''.time().'\')')) {
    ...
    */
    function urlOut(){
        global $tsCore;
        //
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT a_url FROM w_afiliados WHERE aid = \''.intval($_GET['ref']).'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
