      ...
      if( !preg_match("/^[a-zA-Z0-9_-]{4,16}$/", $tsData['user_nick']) ) die('nick: Nombre de usuario inv&aacute;lido');
		...
		if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_miembros` (`user_name`, `user_password`, `user_email`, `user_rango`, `user_registro`) VALUES (\''.$tsCore->setSecure($tsData['user_nick']).'\', \''.$tsCore->setSecure($key).'\', \''.$tsCore->setSecure($tsData['user_email']).'\', '.(empty($tsCore->settings['c_reg_rango']) ? 3 : $tsCore->settings['c_reg_rango']).', \''.$tsData['user_registro'].'\')')){
         ...
         $numero = rand(1,10);
        	...
        	$dirimg = TS_ROOT . '/files/avatares/' . $genero . '/' . $numero . '.jpg';
			$newName = TS_FILES . 'avatar/'. $tsData['user_id'] . '.webp';
			// Creamos y guardamos
			...
			copy($dirimg, TS_FILES . 'avatar/'.$tsData['user_id'].'.webp');
			imagedestroy($img);	
		...
		if(empty($tsCore->settings['c_reg_activate'])){
			...
			return '2: <div class="box_body" style="padding: 10px 0;"><h1 style="font-size:26px;">🎉 Felicidades 🎉</h1><p>Ya eres parte de <b>'.$tsCore->settings['titulo'].'</b>! Ahora puedes disfrutar y compartir todo el contenido que existe en nuestra comunidad sin ninguna restricción, y tu cuenta ha sido activada.<br>¡Muchas gracias!</p><a data-change href="'.$tsCore->settings['url'].'/cuenta/" class="btn btn-gradient-six mt-4 d-inline-block">Comenzar Ahora!</a></div>';
		}
