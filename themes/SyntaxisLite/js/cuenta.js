...
function desactivate(des) {
	if(des == 0) {
		var html = '';
		html += '<b>&#191;Seguro que quiere desativar su cuenta?</b><br>';
		html += 'Si desactiva su cuenta, todo el contenido relacionado a usted dejar&aacute; de ser visible durante un tiempo.<br>';
		html += 'Pasado ese tiempo, la administraci&oacute;n borrar&aacute; todo su contenido y no podr&aacute; recuperarlo.';
		mydialog.show();
...
var cambiar = {
	avamodal: function(usuario, paso) {
		if(paso == 0) {
			$.ajax({
				type: 'POST',
				url: global_data.url + '/upload-cambiar.php',
				data: 'user='+usuario,
				success: function(response) {
					mydialog.show(true);
					mydialog.title('Cambiar avatar');
					mydialog.body('<div class="avatares">'+response+'</div>');
	      		mydialog.buttons(true, true, 'Aceptar cambio', 'cambiar.avamodal('+usuario+', 1)', true, false);
				}
			});
		} else {
			mydialog.show(true);
	...
	avatar: function(usuario) {
		$.ajax({
			...
			success: function(e) {
				...
				$('.avatares').html(e).show();
				$('#ccambiar').show()
			}
		});
	},
	select: function(avatar, id) {
		$.ajax({
			type: 'POST',
			url: global_data.url + '/upload-subir.php',
			data: 'usuario='+avatar+'&id='+id,
	...
	type: function(selection) {
		...
		if(selection == 'pc') {
         ...
         html += '<p>Subir una imagen de perfil desde tu PC.</p>';
         html += '<div class="custom-file custom-renove">'
			html += `<input type="file" class="custom-file-input browse" name="file-avatar" onchange="$(this).next().after().text($(this).val().split('/\/\').slice(-1)[0].replace('C:\/fakepath\',''))" id="file-avatar">`
			html += '<label class="custom-file-label" for="file-avatar" data-browse="Elegir">Seleccionar archivo</label>'
			html += '</div>'
         html += '<button onclick="avatar.upload(this)" class="avatar-next local btn btn-success" style="margin-top: 0.5em;">Subir</button>';
         html += '</div>';
		} else {
			html += '<div class="form-group">'
         html += '<p>Subir una imagen de perfil desde URL.</p>';
			html += '<input type="text" class="form-input" placeholder="Link de tu avatar" name="url-avatar" id="url-avatar" size="45"/>'
			html += '</div>'
			html += '<button onclick="avatar.upload(this);" class="avatar-next button button-success mt-3 url">Subir</button>'
			html += '</div>';
		}
...
var cuenta = {
	...
	chgtab: function (obj) {
		$('ul.menu-tab > li.active').removeClass('active');
	...
	chgpais: function(){
		...
		var el_estado = $('.content-tabs.cuenta select[name=estado]');
	...
	next: function (isprofile) {
		...
		else $('ul.menu-tab > li.active').next().children().click();
	},
		...
		$.ajax({
			...
			success: function (r) {
				...
				if (r.error) {
					...
					if(r.porc != null) {
						$('percente > span').html(r.porc + '%');
						$('.percente').css('width', r.porc + '%');
					}
				}
...
var avatar = {
   ...
   ext: false,
	...
	crop: function (img) {
		...
		mydialog.body('<img class="avatar-crop w-100 mx-auto" src="'+img+'?'+Math.random()+'" onload="mydialog.center()">');
		mydialog.buttons(true, true, 'Cortar avatar', 'avatar.save()', true, false, true, 'Cancelar', 'avatar.close()', true, true);
		$('img.avatar-big').attr('src', img + '?' + Math.random()).on('load', function() {
         $('img.avatar-crop').Jcrop({
            aspectRatio: 1, // Para que mantenga el aspecto de 120x120
            bgColor:'black', // Se puede cambiar el color
				animationDelay: 20,
				allowMove : true, // Especifica si la selección se puede mover
        		outlineOpacity : 0.5, // La opacidad del contorno
        		overlayOpacity : 0.8, // La opacidad superpuesta
				swingSpeed: 3,
         	setSelect: [ 0, 0, 160, 160 ], 
            minSelect: [160, 160], 
            onChange: avatar.preview,
            onSelect: avatar.preview
         })
      });
	},
	...
	close: function () {
      mydialog.body('');
		...
		if (avatar.success) {
         ...
         var img_url = global_data.url + '/files/avatar/' + global_data.user_key + '.webp?reload=true';
         $('#avatar-img, #cambiar-foto, .avatar-head').attr({'src': img_url}).fadeIn();
         $('div.avatar-loading, #ccambiar').hide();
			$('.horizontal, #cambiar').show();
			$('.avatares').html('').hide();
		}
	},
