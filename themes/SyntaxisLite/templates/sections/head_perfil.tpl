		<div class="perfil_nvg">
        	<div>
				...
				<ul class="floatR">
				 <a href="#" onclick="denuncia.nueva('usuario',{$tsInfo.uid}, '', '{$tsInfo.nick}'); return false"><img src="{$tsConfig.images}/icons/warning.png" /></a>
				  {if ($tsUser->is_admod || $tsUser->permisos.mosu) && !$tsInfo.user_baneado}<a href="#" onclick="mod.users.action({$tsInfo.uid}, 'ban', true); return false;"><img src="{$tsConfig.images}/icons/exclamation.png" /></a>{/if}
				 ...
				 <a href="javascript:bloquear({$tsInfo.uid}, {if $tsInfo.block.bid}false{else}true{/if}, 'perfil')" id="bloquear_cambiar">{if $tsInfo.block.bid}<img src="{$tsConfig.images}/icons/power_on.png" />{else}<img src="{$tsConfig.images}/icons/power_off.png" />{/if}</a>
				{if $tsUser->uid != $tsInfo.uid && $tsUser->is_member}
