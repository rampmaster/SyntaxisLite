   ...
   <div class="data-info backdrop-blur-x3">
      ...
      <div class="more-data">
         <span><a href="{$tsConfig.url}/perfil/{$p.user_name}" itemprop="author" itemscope itemtype="http://schema.org/Person">@{$p.user_name}</a>
            <small><span itemprop="datePublished" itemscope itemtype="http://schema.org/DateTime">{$p.post_date|hace}</span> {if $p.post_private}<i feather="lock" class="float-right" style="width:14px;height:14px;"></i>{/if}{if $p.post_sticky} <i feather="paperclip" class="float-right" style="width:14px;height:14px;"></i>{/if}</small>
         </span>
      </div>
   </div>
</div>