                                ...
                                {else}
                                <table cellpadding="0" cellspacing="0" border="0" width="100%" align="center" class="table table-striped">
                                    	<thead>
                                        	<th>ID</th>
                                            <th>Tipo</th>
                                            <th>Texto</th>
                                        ...
                                        <tbody>{foreach from=$tsBlackList.data item=b}
                                        	<tr id="block_{$b.id}">
                                                ...
                                                <td>{$b.value}</td>
                                                <td>{$b.reason}</td>
                                                <td><a href="{$tsConfig.url}/perfil/{$b.user_name}" data-vcard="{$b.user_id}">{$b.user_name}</a></td>
                                                <td>{$b.date|hace}</td>
								...
								{elseif $tsAct == 'editar' || $tsAct == 'nuevo'}
										...
										<fieldset>
                                            ...
                                            {/if}
											<hr />
										 <p><input type="submit" name="{if $tsAct == 'editar'}edit{else}new{/if}" value="{if $tsAct == 'editar'}Guardar{else}Agregar{/if}" class="btn btn-success"/>
										</fieldset>
										</form>
										{/if}
</div>