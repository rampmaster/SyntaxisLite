								...
								{else}
								<table cellpadding="0" cellspacing="0" border="0" class="table table-striped" width="100%" align="center">
									<thead>
										<th>T&iacute;tulo</th>
										<th>Autor</th>
										<th>Moderador</th>
									...
									<tbody>
										...
										<tr id="report_{$p.post_id}">
											...
											<td><a href="{$tsConfig.url}/perfil/{$p.user_name}" data-vcard="{$p.user_id}">{$p.user_name}</a></td>
											<td><a href="{$tsConfig.url}/perfil/{$p.mod_name}" data-vcard="{$p.mod}">{$p.mod_name}</a></td>
